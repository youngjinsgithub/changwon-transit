...
경남·부산·울산 권역의 주요 시(市) 1인당 자동차 등록대수를 비교하여
**차량 의존도가 가장 높은 창원시**를 분석 대상으로 선정.

| 순위 | 도시 | 자동차 (대) | 인구 (명) | 1인당 |
|:---:|:---|---:|---:|---:|
| **1** | **창원시** | 697,261 | 990,898 | **0.704** |
| 2 | 밀양시 | 63,534 | 99,252 | 0.640 |
| 3 | 사천시 | 63,349 | 107,935 | 0.587 |
| 4 | 김해시 | 307,774 | 533,035 | 0.577 |
| 5 | 울산광역시 | 611,789 | 1,091,948 | 0.560 |
| 6 | 양산시 | 196,751 | 361,155 | 0.545 |
| 7 | 진주시 | 182,591 | 335,939 | 0.544 |
| 8 | 거제시 | 114,859 | 231,178 | 0.497 |
| 9 | 부산광역시 | 1,578,413 | 3,241,600 | 0.487 |
| 10 | 통영시 | 55,432 | 116,667 | 0.475 |

> **선정 근거**: 차량 의존도가 높을수록 자가용 → 대중교통 전환 여지가 크며,
> 노선 개편의 잠재 효과(혼잡 완화·온실가스 저감·교통복지)가 가장 큽니다.

- 데이터: KOSIS `1인당 자동차 등록대수(시도/시/군/구)` 통계표 (DT_1YL20731, 2025년)
- 출처: 국토교통부 자동차등록현황보고 + 행정안전부 주민등록인구현황
- 스크립트: [`scripts/step0_select_region.py`](scripts/step0_select_region.py)
- 산출물: [`data/processed/step0_region_ranking.csv`](data/processed/step0_region_ranking.csv)

> ※ 함안군 1.3, 함양군 1.2 등 군지역은 산업·관용차 비중과 인구 규모 차이로
> 시 단위 비교에서 제외함.
